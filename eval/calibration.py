...
def expected_calibration_error(y_true: torch.Tensor, probs: torch.Tensor, n_bins: int = 15) -> float:
    ...
    y_true_np, conf_np, pred_np = y_true.detach().cpu().numpy(), confidences.detach().cpu().numpy(), predictions.detach().cpu().numpy()
    bin_boundaries = np.linspace(0, 1, n_bins + 1)
    ece = 0.0
    for i in range(n_bins):
        in_bin = (conf_np > bin_boundaries[i]) & (conf_np <= bin_boundaries[i+1])
        if in_bin.sum() > 0:
            ece += np.abs(np.mean(conf_np[in_bin]) - np.mean(pred_np[in_bin] == y_true_np[in_bin])) * np.mean(in_bin)
    return ece
...
def plot_reliability_diagram(y_true: torch.Tensor, probs: torch.Tensor, path: str, n_bins: int = 15):
    confidences, predictions = probs.max(dim=-1)
    y_true_np, conf_np, pred_np = y_true.detach().cpu().numpy(), confidences.detach().cpu().numpy(), predictions.detach().cpu().numpy()
    bin_boundaries = np.linspace(0, 1, n_bins + 1)
    bin_acc, bin_conf = [], []
    ...
    for i in range(n_bins):
        in_bin = (conf_np > bin_boundaries[i]) & (conf_np <= bin_boundaries[i+1])
        bin_acc.append(np.mean(pred_np[in_bin] == y_true_np[in_bin]) if in_bin.sum() > 0 else 0.0)
        bin_conf.append(np.mean(conf_np[in_bin]) if in_bin.sum() > 0 else 0.0)
        
    plt.figure(figsize=(6, 6))
    plt.bar(bin_conf, bin_acc, width=0.06, color='skyblue', edgecolor='black')
    plt.plot([0, 1], [0, 1], 'r--')
